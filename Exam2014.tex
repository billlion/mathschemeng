...
\hbox{and} \quad
\bB \,=\, \left( \begin{array}{rr} -1 & 2 \\ 1 & 3 \end{array} \right).\]
...
\[ \bA \bB = \mathrm{det}(\bA)\, \mathbf{I} \]
where $\mathbf{I}$ is the $2 \times 2$ identity matrix. Hence derive a formula for the inverse of $\bA$ when $\mathrm{det} (\bA) \ne 0 $.
 \marks{4}
...
Given
\[ \bA=   \left ( \begin{array}{rrr} 1 & 2 & 3 \\ 3 & 2 & 1 \\ 1 & 0 & 1 \end{array} \right),\quad \bB=   \left ( \begin{array}{rrr} 1 & 0 & 1\\ 3 & 2 & 1 \\  1 & 2 & 3  \end{array} \right) \]
\begin{enumerate}
\item Find a matrix $\bP$ such that $\bP \bA = \bB$.
\item Evaluate $\mathrm{det}(\bA)$, and using part $(a)$ explain why $\mathrm{det}(\bA)=-\mathrm{det}(\bB)$
\end{enumerate}
...
$$ \frac{\dif y}{\dif x}\,=\, -x(9\,+\,y^2)   ; \qquad y=0 \mbox{ when } x=0. $$
\noindent
...
$$  \frac{\dif y}{\dif x}\,+\,\frac{y}{x} = \sin{x}   ; \qquad y=2\>\hbox{ when }\> x=\pi. $$
\marks{5}
...
a function $f(x)$. Hence or otherwise find a {\bf positive} solution to $x = 2\sin{x} $ correct to four significant figures.
\marks{6}
